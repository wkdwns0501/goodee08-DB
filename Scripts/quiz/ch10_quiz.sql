...
-- 정답: 2
-- 1번: 제약 조건(UNIQUE, FK), 정규화의 효과
-- 3번: 튜닝 (인덱스, 쿼리 튜닝 등 물리적 설계)
...
-- 정답: 1
-- 2번: 개체 간의 관계(일대다 관계) 정의는 논리적 모델링 단계에서 수행
-- 3번: 대출 기록 테이블이 중간 테이블이므로 논리적 모델링 단계에서 수행
-- 4번: 컬럼을 정의하는 것은 논리적 모델링 단계에서 수행
...
-- 정답: 2
-- 각 속성은 원자값을 가지고, 같은 속성끼리는 모두 동일한 자료형을 가지며,
-- 각 인스턴스는 고유하므로 제1정규형을 만족

-- 기본키 후보는 member_id, book_id 의 복합키 이며,
