...
-- 정답: 3 (한 번이라도 도서를 대출한 회원들의 이름 목록)
...
-- JOIN 방법
SELECT e.name AS 이름, COUNT(project_id) AS '프로젝트 수'
FROM employees e 
LEFT JOIN employee_projects ep ON e.id = ep.employee_id
GROUP BY e.id, e.name;
-- 직원별로 그룹화(중복 이름[동명이인]이 있을 수도 있으니 id까지 그룹으로 묶어서)
...
-- JOIN 방법
SELECT d.name AS 부서명, e.name AS 직원명
FROM employees e 
...
-- 정답: 
SELECT department_name AS 부서명, COUNT(*) AS 직원수
FROM (
...
-- 정답: 
SELECT name AS 직원명, salary AS 급여
FROM employees
...
-- 정답:
SELECT p.name AS '프로젝트명', COUNT(e.id) AS '참여 직원 수'
FROM employees e 
