...
CREATE TABLE student (
  ...
  student_identity_number VARCHAR(50) NOT NULL UNIQUE,
  student_gender VARCHAR(10), 
  student_address VARCHAR(255),
  student_status VARCHAR(20) DEFAULT '재학',
  student_email VARCHAR(100) UNIQUE,
  student_bank VARCHAR(100),
...
CREATE TABLE lecture (
  ...
  UNIQUE (professor_id, lecture_code, lecture_year, lecture_semester, lecture_section),
  UNIQUE (building_id, lecture_room, lecture_year, lecture_semester)
);
...
CREATE TABLE student_major (
  student_major_id INT AUTO_INCREMENT PRIMARY KEY,
  student_id INT NOT NULL,
  FOREIGN KEY(student_id) REFERENCES student(student_id),
  major_id INT NOT NULL,
  FOREIGN KEY(major_id) REFERENCES major(major_id)
...
CREATE TABLE lecture_history (
  ...
  lecture_id INT NOT NULL,
  FOREIGN KEY(lecture_id) REFERENCES lecture(lecture_id),
  lecture_score DECIMAL(2,1) NULL,
...
CREATE TABLE change_major_history (
  ...
  FOREIGN KEY(from_major_id) REFERENCES major(major_id),
  to_major_id INT NOT NULL,
  FOREIGN KEY(to_major_id) REFERENCES major(major_id)
);
...
CREATE TABLE academic_calendar (
  academic_calendar_id INT AUTO_INCREMENT PRIMARY KEY,
