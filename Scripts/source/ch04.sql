...
-- S(스케일): 소수점 이하 자릿수
-- (참고) DECIMAL 과 FLOAT / DOUBLE 비교
-- DECIMAL : 10진수 기반, 고정 소수점 방식, 연산 속도 상대적으로 느림
-- 		- 주 목적 : 정확성 (돈, 무게, 거리, 재고 수량처럼 반드시 정확한 값이 필요할 때 사용)
-- FLOAT / DOUBLE : 2진수 기반, 부동 소수점 방식, 연산 속도 빠름
--		- 주 목적 : 연산 속도 + 큰 범위 표현 (과학/공학 계산, 통계 처리, 센서 데이터)
-- 				   (DOUBLE이 FLOAT보다 정밀하지만 둘 다 근사값 기반)
...
-- 위 퀴리를 IN 연산자(목록에 포함된 값 찾기)를 활용한 버전으로 다시 작성한다면?
...
FROM transactions 
WHERE (msg = 'Google' or msg = 'Amazon') AND amount > 0; 
-- IN 사용
SELECT AVG(amount) avg 
FROM transactions 
...
SELECT DISTINCT COUNT(*) FROM transactions; -- msg의 개수 27에 대한 중복 제거 (오답)
