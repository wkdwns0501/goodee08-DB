...
-- 주문은 특정 사용자를 참조할 수도 있고, 또는 참조하지 않을 수도 있다.
-- 	   즉, 비회원인 경우에도 주문을 할 수 있다.(0~1)
...
-- 1. 개념적 데이터 모델링(실무에서는 머리로만 그려보고 스킵하는 경우가 많음)
-- 데이터베이스 전문가 뿐만 아니라 비즈니스 담당자, 최종 사용자 등 누가 보더라도 쉽게 이해할 수 있게 작성

-- draw.io(https://app.diagrams.net/)에 실습 (다이어그램 툴)
...
-- 이때 주문과 상품은 다대다 관계(다대다 관계는 직접 연결X)
...
-- 인덱스는 검색 및 정렬 성능을 향상시키는 장점도 있지만,
--  저장 공간을 많이 차지하고 쓰기 성능이 저하되는 단점도 있음
...
CREATE TABLE orders (
	...
	user_id INTEGER NOT NULL, 			-- 사용자 아이디(필수 입력) <- 필수적 관계(일대일)
	PRIMARY KEY (id), 					-- 기본키 지정: id
...
CREATE TABLE payments (
	...
	order_id INTEGER NOT NULL UNIQUE, 	-- 주문 아이디(필수 입력, 고유키 지정) <-- 필수적 관계(일대일)
	PRIMARY KEY (id), 					-- 기본키 지정: id
...
CREATE TABLE order_details (
	...
	-- order_id, product_id이 한 쌍을 이루어서 고유한 값을 가져야만 함
	-- 의미: 하나의 주문에 같은 상품이 중복으로 담기지 않게 하기 위해 설정
	--       (같은 상품을 여러 개 담을 경우 count로 관리)
);
