...
-- 종류: TINYINT, SMALLINT, MEDIUMINT, INTEGER(또는 INT)*, BIGINT(21억이 넘어갈 때 사용)

-- UNSIGNED(언사인드) 제약조건 부여 가능: 음수 값을 허용하지 않는 정수 (<-> SIGNED(사인드): 음수 값 허용)
...
CREATE TABLE files (
	...
	small_thumnail TINYBLOB, -- 작은 이미지 파일(최대 255byte)
...
-- (참고)
-- 자바 웹 개발을 포함한 대무분의 애플리케이션에서는 이미지나 동영상 같은 대용량 파일을
...
-- ENUM 자료형의 예
create table memberships WHERE (
...
CREATE TABLE events(
	...
	event_date DATE, -- 이벤트 일자(YYY-MM-DD)
...
-- 괄호 안에 전달된 여러 문자열을 순서대로 이어 붙여 하나의 문자열로 만듬
SELECT name, price, CONCAT(name, ' / ', price, '원') name_and_price 
FROM orders;

-- CONCAT_WS(구분자, 문자열1, 문자열2, ...): 
-- 		CONCAT과 비슷하지만, 첫 번째 인자로 '구분자'를 받아 각 문자열 사이에 넣어줌
SELECT CONCAT_WS(' - ', name, price, quantity) order_details 
FROM orders; 
...
SELECT SUM(price * quantity) Salse FROM orders WHERE HOUR(created_at) < 12;

-- 4. 특정 범위 필터링하기
-- 1) BETWEEN 연산자
-- 두 값 사이에 속하는지 확인할 때 사용하는 연산자(두 값을 포함하여 찾음)
SELECT * FROM 테이블명 WHERE 컬럼명 BETWEEN 시작_값 AND 마지막_값;

-- 상품 가격이 10,000 ~20,000원 사이에 있는 주문은?
SELECT * FROM orders WHERE price BETWEEN 10000 AND 20000;

-- (참고) 상품 가격이 10,000 ~20,000원 사이가 아닌 주문은? (10,000미만, 20,000초과)
SELECT * FROM orders WHERE price NOT BETWEEN 10000 AND 20000;

-- 주문 시작이 2024-11-15 부터 2024-12-15 사이에 있는 주문의 개수는?
...
-- 첫 글자가 'ㄱ'으로 시작하는 것 찾으려면 유니코드 범위를 활용
-- 가~깋 범위에 해당
SELECT * FROM orders WHERE name >='가' AND name < '나';

-- 주의
SELECT * FROM orders WHERE name LIKE 'ㄱ%'; -- 'ㄱ'으로 시작하는 문자열 검색
